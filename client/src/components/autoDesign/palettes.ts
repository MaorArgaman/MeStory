/**
 * Curated palette library — the color half of the genome. Each palette is a
 * hand-balanced {text, background, accent, muted} set that reads well for a
 * full book interior (legible body, harmonious accent, quiet muted). Palettes
 * are tagged by mood/temperature so an archetype can draw only the families
 * that suit it (a memoir never gets neon; a zine never gets funeral-grey).
 *
 * This is a multiplier: ~60 palettes × the other genome axes = thousands of
 * coherent looks. Add palettes freely — just tag them.
 */
