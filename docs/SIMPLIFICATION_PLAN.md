...
>
> **עדכון מיצוב 2026-07-09:** הוחלט ש-MeStory היא **מערכת עיצוב ספרים**, לא מערכת
> כתיבה. השדרה הראשית החדשה: `העלאה → מבנה → עיצוב → עידון → ייצוא/דפוס`;
> הכתיבה הופכת לכלי משני בתוך פרויקט. ראה `docs/AUTO_DESIGN_PRO_PLAN.md` סעיף 0.
> שלבי הניקיון במסמך זה עדיין תקפים (ובראשם שלב 3 — renderer אחד — שהוא תנאי
> מקדים למוכנות-לדפוס), אבל העיקרון המנחה בסעיף D מוחלף בשדרה החדשה.
